...
- **Hatch dump + JSONC**: `hatch-141.{json,md}` → `design/design_v0_1_structural_hatch.nbcad.jsonc` (via `tools/dump_to_jsonc.py --role hatch`). MoveCopy5→Extrude88∩Combine4→Extrude96/pattern/mirror/Combine13→Revolve1/pattern/mirror/Combine14. Replay (A): shell chapter `--max-fid 290` then hatch (or `--embed-shell`). Blank-doc AABB **138×179.4×79.932** @ mins (-69,-89.7,123) — **XY match**; Z VERIFY vs review **138×179.4×48.0928** @ zmin 154.859. Legacy coupons removed from steps (README only). Exports: `first-prints/native-port/curved-hatch.*`.
...
- **Next**: resolve hatch Z VERIFY (Extrude88/Combine3 at Z123 vs review zmin 154.859); then Combine16 for final shell AABB.

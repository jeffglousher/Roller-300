...
1. **multi_body_m3** (Extrude74): Also targets body 97 (carriage). Caps chapter should cut this cap; carriage chapter already cuts 97.
...
1. Extrude72 new_body blank
2. Extrude73 cut bore
3. Extrude74 cut M3 (cap target; carriage already has carriage-only cut)
4. Extrude144 new_body corner-limit tool
5. Combine18 intersect cap with tool
...
See `input_cap_outer-108.json` for full sketch entity lists, datum sources, and per-feature payloads.

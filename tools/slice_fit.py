...
import json,subprocess,sys,struct,zipfile,xml.etree.ElementTree as ET
ROOT=Path(__file__).resolve().parents[1]
PROFILE=Path(r'C:\Program Files\Bambu Studio\resources\profiles\BBL')
OUT=ROOT/'first-prints'/'drive-end-section'/'slicer-check'/sys.argv[1];OUT.mkdir(parents=True,exist_ok=True)
...
args=[r'C:\Program Files\Bambu Studio\bambu-studio.exe','--debug','3','--arrange','1','--load-settings',str(mp)+';'+str(pp),'--load-filaments',str(fp),'--curr-bed-type','Textured PEI Plate','--slice','0','--export-3mf',str(OUT/(part+'-estimate.3mf')),'--export-settings',str(OUT/(part+'-used-settings.json')),'--outputdir',str(OUT),str(ROOT/'first-prints'/'drive-end-section'/(part+'.stl'))]
...
if True:  # Fixed center avoids the CLI arranger restricting this part to both nozzles' intersection.
 ...
 data=(ROOT/'first-prints'/'drive-end-section'/(part+'.stl')).read_bytes();count=struct.unpack_from('<I',data,80)[0]

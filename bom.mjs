...
sh.mergeCells('A2:O2');sh.getRange('A2').values=[['Owned INJORA INJS035-360 locked. ONE reusable vehicle drive end section. No new servo/programmer purchase. Powered release held.']];
...
if(Math.abs(before-33.04)>0.005)throw Error(`Unexpected first-purchase subtotal ${before}`);
...
await fs.writeFile(path.join(root,'bom-check.json'),JSON.stringify({subtotal_usd:before,owned_quantity_edit_check:true,formula_errors:errors,rows:rows.length,inspect},null,2));
...
await fs.writeFile(path.join(root,'bom-preview.png'),new Uint8Array(await preview.arrayBuffer()));
const changed=await wb.render({sheetName:'BOM',range:`A${last-3}:O${last}`,scale:1,format:'png'});await fs.writeFile(path.join(root,'bom-drive-preview.png'),new Uint8Array(await changed.arrayBuffer()));
